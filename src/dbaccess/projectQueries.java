...
public class projectQueries {
	...
	public void addQueries()
	{
		...
		//8
		queries.add("(SELECT skill_id FROM skill_require WHERE job_code = 1) MINUS (SELECT skill_id FROM knows_skill WHERE person_id = 1);");
		//9
		...
		//17
		queries.add("SELECT name,email FROM person P  WHERE NOT EXISTS( ( SELECT skill_id FROM skill_require WHERE job_code=1 ) MINUS ( SELECT skill_id FROM knows_skill K WHERE P.person_id = K.person_id) );");
		//18
		queries.add("SELECT person_id FROM knows_skill NATURAL JOIN skill_require GROUP BY(person_id) HAVING  (SELECT COUNT(*) FROM skill_require WHERE job_code=1  )- COUNT(person_id) =1;");
		//19
		queries.add("WITH missing_one AS  ( SELECT skill_id  FROM knows_skill NATURAL JOIN skill_require GROUP BY(skill_id) HAVING  (SELECT COUNT(*) FROM skill_require WHERE job_code=1  )- COUNT(person_id) =1)  SELECT skill_id, COUNT(*) AS personsMissing_count FROM missing_one  GROUP BY (skill_id) ORDER BY personsMissing_count ASC;");
		//20
		queries.add("WITH number_needs(person_id,needs) AS (  (SELECT person_id,((SELECT COUNT(*) FROM skill_require WHERE job_code=1)-count(person_id)) as needs FROM knows_skill natural join skill_require GROUP BY (person_id) ) ) SELECT person_id,needs FROM  number_needs WHERE needs= (SELECT MIN(needs)  FROM number_needs);");
		//21
		queries.add("WITH number_needs(person_id,needs) AS ( (SELECT person_id,((SELECT COUNT(*) FROM skill_require WHERE job_code=1)-count(person_id)) as needs FROM knows_skill natural join skill_require GROUP BY (person_id) ) ) SELECT person_id,needs FROM number_needs WHERE needs <=2 ORDER BY needs ASC;");
		//22
		queries.add("WITH missing_one AS  ( SELECT skill_id  FROM knows_skill NATURAL JOIN skill_require GROUP BY(skill_id) HAVING  (SELECT COUNT(*) FROM skill_require WHERE job_code=1  )- COUNT(person_id) =1) SELECT skill_id, COUNT(*) AS personsMissing_count FROM missing_one  GROUP BY (skill_id) ORDER BY personsMissing_count DESC;");
		//23
	...
	{
		return (String[]) this.questions.toArray();
	}
